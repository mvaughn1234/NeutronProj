...
exports.runConfigs = (socket, Configs) => {
    ...
    promises.then(configs =>
        Promise.all(configs).then(configSet => {
            ...
            // const generator = spawn('. /home/ubuntu/Downloads/geant/build/geant4make.sh && python3 && pwd', ['/home/student/geant4/NeutronProj/scripts/py_scripts/runGenerateConfigs.py', fullPath], {shell: true});
            const generator = spawn('pwd');
            generator.stdout.on('data', (stdout) => {
                socket.emit('runConfigsClient', 'stdout: ' + stdout+'\n');
                console.log(`stdout: ${stdout}`)
            });
            generator.stderr.on('data', (stderr) => {
                socket.emit('runConfigsClient', 'stderr: ' + stderr+'\n');
                console.log(`stderr: ${stderr}`)
            });
            generator.on('close', (code) => {
                socket.emit('runConfigsClient', 'exited with err: ' + code + '\n');
