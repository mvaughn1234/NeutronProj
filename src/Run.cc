...
void Run::EndOfRun()
{
  ...
  G4double rad;
  rad = fDetector->GetRadius1() +
        fDetector->GetRadius2() +
        fDetector->GetRadius3() +
        fDetector->GetRadius4();

  //run condition
  //
  G4Material* material = fDetector->GetMaterial1();
  G4double density = material->GetDensity();

  G4String Particle = fParticle->GetParticleName();
  G4cout << "\n The run is " << numberOfEvent << " "<< Particle << " of "
         << G4BestUnit(fEkin,"Energy") << " through "
         << G4BestUnit(rad,"Length") << " of "
