...
class BruteForce:
    ...
    def mult_sets(self, mats, data, mode):
        ...
        if (mode == 0):
            perms = list(map(lambda x: np.linalg.multi_dot(np.ndarray.astype(x,dtype=float)),perms))
        else:
            perms = list(perms)
        # else:
    ...
    def generateListOfCombinations(self):
        ...
        t = [0,0,0,0]
        s = timer()
        for j in range(0, np.shape(matPerms)[0], 1):
            if j%10 == 0:
                print('here, ', j, ' ', t)
            matSet = matPerms[j];
            ...
            processed = np.linalg.norm(np.array(np.dot(res,eIn)-eDes), axis=1)
            t[2] = t[2] + timer() - s; s = timer();
            imin = np.argmin(processed)
            lock.acquire(True)
            if processed[imin] < self.analysisData.get('curDiff'):
                curSup = sup[imin]
                eOut = np.dot(res[imin], eIn)
                self.analysisData.set('eOut', eOut.tolist())
                self.analysisData.set('iteration', j)
                self.analysisData.set('curDiff', processed[imin])
